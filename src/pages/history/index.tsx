...
import type { Task, CloudResponse, TaskListResponse } from '@/types';
import { STATUS_MAP, PRIORITY_STYLE, TIME_FILTERS, getScoreColor } from '@/constants';
...
import { ChevronRight, Award, Send, User, Trash2, CalendarDays, Loader } from 'lucide-react-taro';
...
export default function History() {
  ...
  const loadTasks = useCallback(async (refresh = false) => {
    ...
    try {
      ...
      const res = await callFunction<CloudResponse<TaskListResponse>>('task-history', params);
  ...
  }, [openid, currentTab, page, timeFilter, selectedDateRange]);
  ...
  // 获取当前 Tab 图标
  const CurrentTabIcon = TAB_CONFIG.find(t => t.value === currentTab)?.icon || Send;
            ...
            return (
              <View
                ...
                className={`flex-1 py-3 rounded-xl flex flex-col items-center justify-center transition-all ${
                  isActive ? 'bg-blue-500' : 'bg-gray-50'
                }`}
          ...
          {loading && tasks.length === 0 ? (
            ...
            <>
              ...
              {!hasMore && tasks.length > 0 && (
                <View className="flex justify-center py-4">
                  <Text className="text-gray-400 text-sm">已加载全部</Text>
                </View>
              )}
            </>
          )}
